...
vk::PhysicalDevice choose_physical_device(const vk::Instance& instance) {
    ...
    message << "There are " << availableDevices.size()
...
vk::Device create_logical_device(vk::PhysicalDevice physicalDevice,
    ...
    vk::DeviceCreateInfo deviceInfo = vk::DeviceCreateInfo(
        vk::DeviceCreateFlags(), static_cast<uint32_t>(queueCreateInfo.size()),
        queueCreateInfo.data(), static_cast<uint32_t>(enabledLayers.size()),
        enabledLayers.data(), static_cast<uint32_t>(deviceExtensions.size()),
        deviceExtensions.data(), &deviceFeatures);

    try {
        vk::Device device = physicalDevice.createDevice(deviceInfo);
        vkLogging::Logger::get_logger()->print(
            "GPU has been successfully abstracted!");
        return device;
