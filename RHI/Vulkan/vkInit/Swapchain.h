...
/**
	...
	*/
vk::Extent2D choose_swapchain_extent(uint32_t width, uint32_t height,
                                     vk::SurfaceCapabilitiesKHR capabilities) {
...
SwapChainBundle create_swapchain(vk::Device logicalDevice,
    ...
    std::vector<vk::Image> images =
        logicalDevice.getSwapchainImagesKHR(bundle.swapchain);
    ...
    for (size_t i = 0; i < images.size(); ++i) {
        ...
        bundle.frames[i].imageView = vkImage::make_image_view(
            logicalDevice, images[i], format.format,
