...
vkMesh::ObjMesh::ObjMesh(const char* objFilepath, const char* mtlFilepath,
    ...
    if (newPath == "") {
        std::cout << "[ERROR] Failed to locating file: " << objFilepath
                  << std::endl;
    } else {
        objPath = newPath;
    }

    newPath = vkUtil::relativePathSearching(mtlFilepath);
    if (newPath == "") {
        std::cout << "[ERROR] Failed to locating file: " << mtlFilepath
                  << std::endl;
    } else {
        mtlPath = newPath;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (!words[0].compare("Kd")) {
            brushColor = glm::vec3(std::stof(words[1]), std::stof(words[2]),
                                   std::stof(words[3]));
