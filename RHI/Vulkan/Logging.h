...
std::vector<std::string> log_alpha_composite_bits(
    ...
    if (bits & vk::CompositeAlphaFlagBitsKHR::ePreMultiplied) {
        result.push_back(
            "pre multiplied (alpha expected to already be multiplied in "
            "image)");
    }
    if (bits & vk::CompositeAlphaFlagBitsKHR::ePostMultiplied) {
        result.push_back(
            "post multiplied (alpha will be applied during composition)");
    }
...
std::vector<std::string> log_image_usage_bits(vk::ImageUsageFlags bits) {
    ...
    if (bits & vk::ImageUsageFlagBits::eTransferSrc) {
        result.push_back(
            "transfer src: image can be used as the source of a transfer "
            "command.");
    }
    if (bits & vk::ImageUsageFlagBits::eTransferDst) {
        result.push_back(
            "transfer dst: image can be used as the destination of a transfer "
            "command.");
    }
