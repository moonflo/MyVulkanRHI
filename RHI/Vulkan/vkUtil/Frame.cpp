...
void vkUtil::SwapChainFrame::make_descriptor_resources() {
    ...
    cameraVectorWriteLocation = logicalDevice.mapMemory(
        cameraVectorBuffer.bufferMemory, 0, sizeof(CameraVectors));

    input.size = sizeof(CameraMatrices);
    cameraMatrixBuffer = createBuffer(input);

    cameraMatrixWriteLocation = logicalDevice.mapMemory(
        cameraMatrixBuffer.bufferMemory, 0, sizeof(CameraMatrices));

    input.size = 1024 * sizeof(glm::mat4);
    input.usage = vk::BufferUsageFlagBits::eStorageBuffer;
    modelBuffer = createBuffer(input);

    modelBufferWriteLocation = logicalDevice.mapMemory(
...
void vkUtil::SwapChainFrame::make_depth_resources() {
    ...
    depthFormat = vkImage::find_supported_format(
        physicalDevice, {vk::Format::eD32Sfloat, vk::Format::eD24UnormS8Uint},
    ...
    depthBufferView = vkImage::make_image_view(
        logicalDevice, depthBuffer, depthFormat,
...
void vkUtil::SwapChainFrame::record_write_operations() {
	...
	*/
    vk::WriteDescriptorSet cameraVectorWriteOp, cameraMatrixWriteOp,
        ssboWriteOp;

    cameraVectorWriteOp.dstSet = descriptorSet[pipelineType::SKY];
    cameraVectorWriteOp.dstBinding = 0;
    cameraVectorWriteOp.dstArrayElement =
    ...
    cameraMatrixWriteOp.dstArrayElement =
        0;  //byte offset within binding for inline uniform blocks
