...
vk::DeviceMemory vkImage::make_image_memory(ImageInputChunk input,
                                            vk::Image image) {

    vk::MemoryRequirements requirements =
        input.logicalDevice.getImageMemoryRequirements(image);

    vk::MemoryAllocateInfo allocation;
    allocation.allocationSize = requirements.size;
    allocation.memoryTypeIndex = vkUtil::findMemoryTypeIndex(
        input.physicalDevice, requirements.memoryTypeBits,
...
void vkImage::transition_image_layout(ImageLayoutTransitionJob transitionJob) {
	...
	*/
    ...
    transitionJob.commandBuffer.pipelineBarrier(sourceStage, destinationStage,
...
void vkImage::copy_buffer_to_image(BufferImageCopyJob copyJob) {
	...
	*/
    ...
    copy.imageExtent = vk::Extent3D(copyJob.width, copyJob.height, 1);

    copyJob.commandBuffer.copyBufferToImage(
        copyJob.srcBuffer, copyJob.dstImage,
        vk::ImageLayout::eTransferDstOptimal, copy);

    vkUtil::endJob(copyJob.commandBuffer, copyJob.queue);
}

vk::ImageView vkImage::make_image_view(vk::Device logicalDevice,
