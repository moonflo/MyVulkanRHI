...
void vkImage::Texture::load(TextureInputChunk input) {
    ...
    std::string fileString(input.filenames[0]);
    std::cout << "[INFO] Start searching file:" << fileString << std::endl;

    std::string filePath = vkUtil::relativePathSearching(fileString);
    if (filePath == "") {
...
void vkImage::Texture::populate() {
    ...
    //...then fill it,
    void* writeLocation =
        logicalDevice.mapMemory(stagingBuffer.bufferMemory, 0, input.size);
