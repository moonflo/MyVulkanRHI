...
vkImage::CubeMap::CubeMap(TextureInputChunk input) {
    ...
    filePaths.resize(input.filenames.size());

    for (int i = 0; i < input.filenames.size(); i++) {
        std::string fileString(input.filenames[i]);
        std::cout << "[INFO] Start searching file:" << fileString << std::endl;
        filePaths[i] = vkUtil::relativePathSearching(fileString);
        if (filePaths[i] == "") {
            std::cout << "[ERROR] Failed to locating file: " << fileString
                      << std::endl;
        } else {
            this->filenames.push_back(filePaths[i].c_str());
        }
    }

    for (int i = 0; i < 6; i++) {
        std::cout << "[INFO] Second time print filenames :"
                  << this->filenames[i] << std::endl;
    }
...
void vkImage::CubeMap::load() {
    for (int i = 0; i < 6; ++i) {
        ...
        if (!pixels) {
            vkLogging::Logger::get_logger()->print_list(
                {"Unable to load: ", filenames[i]});
        }
    }
}
...
void vkImage::CubeMap::populate() {
    ...
    //...then fill it,
    for (int i = 0; i < 6; ++i) {
        void* writeLocation = logicalDevice.mapMemory(
            stagingBuffer.bufferMemory, image_size * i, image_size);
...
void vkImage::CubeMap::make_view() {
    imageView = make_image_view(
        logicalDevice, image, vk::Format::eR8G8B8A8Unorm,
...
void vkImage::CubeMap::make_descriptor_set() {
    ...
    descriptorSet =
        vkInit::allocate_descriptor_set(logicalDevice, descriptorPool, layout);
