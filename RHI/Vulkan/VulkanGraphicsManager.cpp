...
VulkanGraphicsManager::VulkanGraphicsManager(int width, int height,
                                             GLFWwindow* window) {
    ...
    vkLogging::Logger::get_logger()->print(
        "Making a graphics VulkanGraphicsManager...");
...
void VulkanGraphicsManager::make_device() {
...
* Make a swapchain
*/
void VulkanGraphicsManager::make_swapchain() {
...
*/
void VulkanGraphicsManager::recreate_swapchain() {
...
void VulkanGraphicsManager::make_descriptor_set_layouts() {
    ...
    meshSetLayout[pipelineType::SKY] =
        vkInit::make_descriptor_set_layout(device, bindings);
    meshSetLayout[pipelineType::STANDARD] =
        vkInit::make_descriptor_set_layout(device, bindings);
}

void VulkanGraphicsManager::make_pipelines() {

    vkInit::PipelineBuilder pipelineBuilder(device);

    //Sky
    pipelineBuilder.set_overwrite_mode(false);
    pipelineBuilder.specify_vertex_shader("Asset/Shaders/sky_vertex.spv");
    pipelineBuilder.specify_fragment_shader("Asset/Shaders/sky_fragment.spv");
    ...
    pipelineBuilder.specify_vertex_shader("Asset/Shaders/vertex.spv");
    pipelineBuilder.specify_fragment_shader("Asset/Shaders/fragment.spv");
    pipelineBuilder.specify_swapchain_extent(swapchainExtent);
...
*/
void VulkanGraphicsManager::make_framebuffers() {
...
void VulkanGraphicsManager::finalize_setup() {
...
void VulkanGraphicsManager::make_frame_resources() {
    ...
    for (vkUtil::SwapChainFrame& frame : swapchainFrames) {
        ...
        frame.descriptorSet[pipelineType::SKY] =
            vkInit::allocate_descriptor_set(device, frameDescriptorPool,
                                            frameSetLayout[pipelineType::SKY]);
        frame.descriptorSet[pipelineType::STANDARD] =
            vkInit::allocate_descriptor_set(
                device, frameDescriptorPool,
                frameSetLayout[pipelineType::STANDARD]);
...
void VulkanGraphicsManager::make_assets() {
    ...
    std::unordered_map<meshTypes, std::vector<const char*>> model_filenames = {
        {meshTypes::GROUND,
         {"Asset/Models/ground.obj", "Asset/Models/ground.mtl"}},
        {meshTypes::GIRL, {"Asset/Models/girl.obj", "Asset/Models/girl.mtl"}},
        {meshTypes::SKULL,
         {"Asset/Models/skull.obj", "Asset/Models/skull.mtl"}}};
    ...
    std::unordered_map<meshTypes, std::vector<const char*>> filenames = {
        {meshTypes::GROUND, {"Asset/Textures/ground.jpg"}},
        {meshTypes::GIRL, {"Asset/Textures/none.png"}},
        {meshTypes::SKULL, {"Asset/Textures/skull.png"}}};
    ...
    textureInfo.filenames = {{
        ...
        "Asset/Textures/sky_left.png",    //y+
        "Asset/Textures/sky_right.png",   //y-
        "Asset/Textures/sky_bottom.png",  //z+
...
void VulkanGraphicsManager::prepare_frame(uint32_t imageIndex, Scene* scene) {
    ...
    memcpy(_frame.cameraVectorWriteLocation, &(_frame.cameraVectorData),
    ...
    memcpy(_frame.cameraMatrixWriteLocation, &(_frame.cameraMatrixData),
    ...
    for (std::pair<meshTypes, std::vector<glm::vec3>> pair : scene->positions) {
        for (glm::vec3& position : pair.second) {
            _frame.modelTransforms[i++] =
                glm::translate(glm::mat4(1.0f), position);
        }
    }
    memcpy(_frame.modelBufferWriteLocation, _frame.modelTransforms.data(),
           i * sizeof(glm::mat4));

    _frame.write_descriptor_set();
}

void VulkanGraphicsManager::prepare_scene(vk::CommandBuffer commandBuffer) {

    vk::Buffer vertexBuffers[] = {meshes->vertexBuffer.buffer};
    vk::DeviceSize offsets[] = {0};
    commandBuffer.bindVertexBuffers(0, 1, vertexBuffers, offsets);
    commandBuffer.bindIndexBuffer(meshes->indexBuffer.buffer, 0,
                                  vk::IndexType::eUint32);
}

void VulkanGraphicsManager::record_draw_commands_sky(
    vk::CommandBuffer commandBuffer, uint32_t imageIndex, Scene* scene) {

    vk::RenderPassBeginInfo renderPassInfo = {};
    renderPassInfo.renderPass = renderpass[pipelineType::SKY];
    renderPassInfo.framebuffer =
        swapchainFrames[imageIndex].framebuffer[pipelineType::SKY];
    ...
    commandBuffer.bindDescriptorSets(
        vk::PipelineBindPoint::eGraphics, pipelineLayout[pipelineType::SKY], 0,
        swapchainFrames[imageIndex].descriptorSet[pipelineType::SKY], nullptr);
...
void VulkanGraphicsManager::record_draw_commands_scene(
    vk::CommandBuffer commandBuffer, uint32_t imageIndex, Scene* scene) {

    vk::RenderPassBeginInfo renderPassInfo = {};
    renderPassInfo.renderPass = renderpass[pipelineType::STANDARD];
    renderPassInfo.framebuffer =
        swapchainFrames[imageIndex].framebuffer[pipelineType::STANDARD];
    renderPassInfo.renderArea.offset.x = 0;
    renderPassInfo.renderArea.offset.y = 0;
    renderPassInfo.renderArea.extent = swapchainExtent;
    ...
    commandBuffer.bindDescriptorSets(
        ...
        pipelineLayout[pipelineType::STANDARD], 0,
        swapchainFrames[imageIndex].descriptorSet[pipelineType::STANDARD],
        nullptr);
    ...
    for (std::pair<meshTypes, std::vector<glm::vec3>> pair : scene->positions) {
        render_objects(commandBuffer, pair.first, startInstance,
...
void VulkanGraphicsManager::render_objects(vk::CommandBuffer commandBuffer,
                                           meshTypes objectType,
    ...
    materials[objectType]->use(commandBuffer,
                               pipelineLayout[pipelineType::STANDARD]);
    commandBuffer.drawIndexed(indexCount, instanceCount, firstIndex, 0,
                              startInstance);
    startInstance += instanceCount;
}

void VulkanGraphicsManager::render(Scene* scene) {
...
*/
void VulkanGraphicsManager::cleanup_swapchain() {
