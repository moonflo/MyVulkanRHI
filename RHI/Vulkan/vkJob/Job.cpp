...
vkJob::MakeModel::MakeModel(vkMesh::ObjMesh& mesh, const char* objFilepath,
                            const char* mtlFilepath, glm::mat4 preTransform)
    : mesh(mesh) {
...
void vkJob::MakeModel::execute(vk::CommandBuffer commandBuffer,
                               vk::Queue queue) {
    mesh.load(objFilepath, mtlFilepath, preTransform);
    status = JobStatus::COMPLETE;
}

vkJob::MakeTexture::MakeTexture(vkImage::Texture* texture,
                                vkImage::TextureInputChunk textureInfo)
    : texture(texture) {
    this->textureInfo = textureInfo;
}

void vkJob::MakeTexture::execute(vk::CommandBuffer commandBuffer,
                                 vk::Queue queue) {
